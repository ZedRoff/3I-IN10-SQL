...
SELECT c.id, c.nom, c.prénom, COUNT(liv.id) AS nb_commandes
FROM Client c
LEFT JOIN Livraison liv ON c.id = liv.id_client
GROUP BY c.id, c.nom, c.prénom;
...
--LE pire livreur
SELECT l.id, l.nom, COUNT(*) AS nb_retards
FROM Livreur l
JOIN Livraison liv ON l.id = liv.id_livreur
WHERE liv.retard = TRUE
GROUP BY l.id, l.nom
...
FROM Pizza p
JOIN Livraison liv ON p.id = liv.id_pizza
GROUP BY p.id, p.nom
ORDER BY nb_commandes DESC
LIMIT 1;
